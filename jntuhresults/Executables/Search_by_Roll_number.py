...
class Results:
        ...
        try:
            ...
            for data in value:
                if('DETAILS' in data):
                    ...
                    total=total+(0 if(value[data]['subject_grade']=='F' or value[data]['subject_grade']=='Ab') else int(grades_to_gpa[value[data]['subject_grade']]))*float(value[data]['subject_credits'])
            ...
            self.deta["Results"][code]["backlogs"]=backlogs
        except:
            pass
    ...
    def scraping_the_grades(self,code,soup):
        try:
            ...
            internal_index = table2_column_names.index("INTERNAL")
            external_index = table2_column_names.index("EXTERNAL")
            total_index = table2_column_names.index("TOTAL")
            ...
            for row in table2:
                ...
                self.deta["Results"][code][subject_code]["subject_internal"]=subject_internal
                self.deta["Results"][code][subject_code]["subject_external"]=subject_external
                self.deta["Results"][code][subject_code]["subject_total"]=subject_total
